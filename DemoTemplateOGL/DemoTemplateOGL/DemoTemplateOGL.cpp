...
void centerCursor(HWND hWnd)
{
    // Obtener el rectángulo del cliente de la ventana
    RECT rect;
    GetClientRect(hWnd, &rect);

    // Calcular la posición del centro de la ventana
    int centerX = (rect.right - rect.left) / 2;
    int centerY = (rect.bottom - rect.top) / 2;

    // Convertir la posición del centro de la ventana a coordenadas de pantalla
    POINT pt = { centerX, centerY };
    ClientToScreen(hWnd, &pt);

    // Establecer la posición del cursor en el centro de la ventana
